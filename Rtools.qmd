...
[**Disclaimer!** Please note that this content is a work in progress and has not undergone a formal peer review process yet. Therefore, it should be considered as preliminary information shared with individuals who intend to use it critically ;-)]{style="color:red;"}
...
get_RLI <- function(range, # a multi polygons sf object of the species range
                    ...
                    fragment.inclusivity = 0.1, # the lowest this value, the highest is the proportion of fragments considered for range calculation.
                    ...
                    very.graphical = F) # plot each fragment under examination and its candidate paths
{
  ...
  orirange <- st_transform(orirange, '+proj=utm +zone=32 +datum=WGS84 +units=m +no_defs') #reproject to UTM zone 32
  ...
  for (frag in 1:nrow(rprp)) {
    ...
    polyi <- rprp[frag,]
    #apply Behrman transformation, lon_0 set depending upon longitudinal extension (correcting for distortion)
    ...
    check.allocation <- extract(playground,rallyrasters_spatial)
    ...
    for (l in 1:nrow(egrid)) {
      linee[[l]] <- gdistance::shortestPath(tr1, 
                                            ...
                                            output="SpatialLines") %>% st_as_sf()
      if(very.graphical){plot(linee[[l]], add=T, col=vcol[l],lwd=2, lty=1)}
      linee[[l]] <- linee[[l]] %>% st_transform(crs = st_crs(orirange))
